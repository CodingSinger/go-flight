...
var parseJson = function (data,node,showDialog,button) {
    ...
    for(var i=0;i<lists.length;i++) {
        ...
        var d = Date.parse(lists[i].Time);
        d = new Date(d);
        var date = dateFormat(d);
        node.html(temp + "<tr class=\"gradeA\">" + "<td>" + lists[i].Fid + "</td>" + "<td>"+lists[i].Departure+"</td>" + "<td class=\"hidden-xs\">"+lists[i].Destination+"</td>" + "<td class=\"center\">"+date+"</td>" + "<td class=\"center hidden-xs\">"+lists[i].Remainer+"</td>" +"<td class=\"center hidden-xs\">"+lists[i].Price+"</td>"+ "<td><button  data-id="+i+" class=\"buy\" "+flag+">"+button+"</button></td>"+ "</tr>");
    ...
    function doclick() {
        ...
        var d = sessionStorage.getItem("lists");
        var obj = JSON.parse(d);

        var flight = obj[id];

        sessionStorage.setItem("fid",flight.Fid);

        $(".descrip").html(flight.Departure+"----->"+flight.Destination+"   ("+flight.Fid+")");
        // localStorage.setItem("buyflight",fid);

        var d = Date.parse(flight.Time);
        d = new Date(d);
        var datew = dateFormat(d);
        $("#flytime2").val(datew);
...
function parseJson2(data,node,table) {
    var lists = data.Include;
    var temp;
    node.html("");
    for(var i=0;i<lists.length;i++) {
        ...
        node.html(temp + "<tr class=\"gradeA\">" + "<td>" + lists[i].Fid + "</td>" + "<td>"+lists[i].Departure+"</td>" + "<td class=\"hidden-xs\">"+lists[i].Destination+"</td>" + "<td class=\"center\">"+lists[i].Time+"</td>" + "<td class=\"center hidden-xs\">"+lists[i].Remainer+"</td>" +"<td class=\"center hidden-xs\">"+lists[i].Price+"</td>"+ "<td><button  data-fid="+lists[i].Fid+" class=\"buy\" >乘客信息</button></td>"+ "</tr>");
    ...
    function init() {
        var trs =document.getElementsByClassName("buy");
        for (var i = 0; i < trs.length; i++)
            trs[i].onclick = queryAndShowTable;
    }//获取tr节点，并循环卫每个tr节点添加双击事件


    function queryAndShowTable() {
        //获取Id
        var fid = $(this).attr("data-fid");


        $.get("http://localhost:9090/fly/queryPassengers",{
            'fid':fid
            ...
            if (data.State == 0){
                ...
                for(var i=0;i<lists.length;i++) {
                    ...
                    table.html(temp + "<tr class=\"gradeA\">" + "<td>" + lists[i].Fid + "</td>" + "<td>"+lists[i].Departure+"</td>" + "<td class=\"hidden-xs\">"+lists[i].Destination+"</td>" + "<td class=\"center\">"+lists[i].Time+"</td>" + "<td class=\"center hidden-xs\">"+lists[i].Remainer+"</td>" +"<td class=\"center hidden-xs\">"+lists[i].Price+"</td>"+ "<td><button  data-id="+i+" class=\"buy\" "+flag+">"+button+"</button></td>"+ "</tr>");
